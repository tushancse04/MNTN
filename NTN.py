...
class NeuralTensorLayer(Layer):
    ...
    def fit(self,x_train1,x_train2,y_train,x_val1,x_val2,y_val):
        ...
        ny_val = np.array([[0,1] if y == 0 else [1,0] for y in y_val])
        ny_train = np.array([[0,1] if y == 0 else [1,0] for y in y_train])
        ...
        g = Dense(output_dim=2)(BilinearLayer)
        ...
        #model.summary()
        model.fit([x_train1, x_train2], ny_train, epochs=5,
                  validation_data=([x_val1, x_val2], ny_val))

        r = model.predict([x_val1, x_val2])
        #plot = tsne_plot()
        #plot.fit_tsne(r,x_val1,x_val2,y_val)
        r = [x[0]  for x in r]
        s = roc_auc_score(y_val,r)
        r = [False if r[i] < .5 else True for i in range(len(r))]
        print('ntn : ',s)
        ofile = open('webkb/out/out.txt','a')
        ofile.write(str(s) + '\n')
        ofile.close()
        return r
